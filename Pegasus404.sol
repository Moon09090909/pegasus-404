...
abstract contract ERC404 is Ownable2Step {
    ...
    mapping(uint256 => uint256) internal _metadataIndex;
    ...
    function _mint(address to) internal virtual {
        ...
        uint metadataId = uint(keccak256(abi.encodePacked(id))) % 98999;
        _metadataIndex[id] = metadataId;
...
contract Pegasus404 is ERC404, Pausable, ReentrancyGuard {
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        uint256 _id = _metadataIndex[id];
        return bytes(baseTokenURI).length > 0 ? string.concat(baseTokenURI, _id.toString(), ".json") : "";
    }
